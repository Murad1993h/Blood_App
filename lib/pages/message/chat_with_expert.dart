...
class ChatWithExpert extends StatefulWidget {
  ...
  @override
  State<ChatWithExpert> createState() => _ChatWithExpertState();
}
...
class _ChatWithExpertState extends State<ChatWithExpert> {
  ...
  void _translateEngToThai() async {
    final request = CompleteText(prompt: translateEngToThai(word: _txtWord.text.toString()), maxTokens: 200, model: TextDavinci3Model());
  ...
  void gptFunctionCalling() async {
    List<Messages> messages = [
      Messages(role: Role.user, content: "What is the weather like in Boston?", name: "get_current_weather"),
    ];
    // final request = ChatCompleteText(messages: messages, maxToken: 200, model: Gpt4ChatModel());
    //
    ...
    final request = ChatCompleteText(
        ...
        functions: [
          FunctionData(name: "get_current_weather", description: "Get the current weather in a given location", parameters: {
            "type": "object",
            "properties": {
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            TextField(
              decoration: const InputDecoration(border: InputBorder.none, enabledBorder: InputBorder.none, disabledBorder: InputBorder.none),
              controller: _txtWord,
              maxLines: 6,
